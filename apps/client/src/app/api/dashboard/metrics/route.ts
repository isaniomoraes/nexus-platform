...
export async function GET() {
  ...
  type W = { is_active: boolean; time_saved_per_execution: number | null; money_saved_per_execution: number | null; execution_count: number | null }
  const list = (workflows ?? []) as W[]
  const activeWorkflows = list.filter((w) => w.is_active).length
  const timeSavedTotal = list.reduce((sum, w) => sum + (w.time_saved_per_execution ?? 0) * (w.execution_count ?? 0), 0)
  const moneySavedTotal = list.reduce((sum, w) => sum + (w.money_saved_per_execution ?? 0) * (w.execution_count ?? 0), 0)
