...
export async function POST(request: Request) {
  ...
  if (auth.user) {
    ...
    await supabase.from('users').upsert({ id: auth.user.id, email: parse.data.email, name: parse.data.name, role: 'admin' })
    await supabase.auth.updateUser({ data: { user_role: 'admin', client_id: null, assigned_clients: [] } })
  }
  return response
}
